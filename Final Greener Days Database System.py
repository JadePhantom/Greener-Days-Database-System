...
def create_tables(): 
    cursor.execute('''CREATE TABLE IF NOT EXISTS Customer (
                    Customer_ID TEXT CHAR(20) PRIMARY KEY CHECK (Customer_ID GLOB 'C[0-9][0-9][0-9][0-9][0-9]'),
                    Customer_First_Name TEXT CHAR(100) NOT NULL CHECK (length(Customer_First_Name) <= 100 AND Customer_First_Name GLOB '[A-Za-z]*'),
                    Customer_Last_Name TEXT CHAR(100) NOT NULL CHECK (length(Customer_Last_Name) <= 100 AND Customer_Last_Name GLOB '[A-Za-z]*'),
                    Birth_Date DATE NOT NULL CHECK (Birth_Date LIKE '__/__/____'),
                    Address TEXT CHAR(100) NOT NULL CHECK (length(Address) <= 100),
                    Customer_Phone TEXT CHAR(20) NOT NULL UNIQUE CHECK (Customer_Phone GLOB '04[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]'),
                    Email TEXT CHAR(100) NOT NULL UNIQUE CHECK (length(Email) <= 100 AND Email LIKE '%@%.%') 
                    )''')
    
    cursor.execute('''CREATE TABLE IF NOT EXISTS Booking (
                    Booking_ID TEXT CHAR(20) PRIMARY KEY CHECK (Booking_ID GLOB 'B[0-9][0-9][0-9][0-9][0-9]'),
                    Booking_Date DATE NOT NULL CHECK (Booking_Date LIKE '__/__/____'),
                    Customer_ID TEXT CHAR(20) NOT NULL,
                    FOREIGN KEY (Customer_ID) REFERENCES Customer(Customer_ID),
                    UNIQUE (Booking_Date, Customer_ID)
                    )''')
    
    cursor.execute('''CREATE TABLE IF NOT EXISTS Service (
                    Service_ID TEXT CHAR(20) PRIMARY KEY CHECK (Service_ID GLOB 'S[0-9][0-9][0-9][0-9][0-9]'),
                    Service_Type TEXT CHAR(50) NOT NULL UNIQUE CHECK (length(Service_Type) <= 50 AND Service_Type GLOB '[A-Za-z ]*'),
                    ...
                    Booking_ID TEXT CHAR(20) NOT NULL,
                    FOREIGN KEY (Booking_ID) REFERENCES Booking(Booking_ID)
                    )''')
    
    cursor.execute('''CREATE TABLE IF NOT EXISTS Employee (
                    Employee_ID TEXT CHAR(20) PRIMARY KEY CHECK (Employee_ID GLOB 'E[0-9][0-9][0-9][0-9][0-9]'),
                    Employee_First_Name TEXT CHAR(100) NOT NULL CHECK (length(Employee_First_Name) <= 100 AND Employee_First_Name GLOB '[A-Za-z]*'),
                    Employee_Last_Name TEXT CHAR(100) NOT NULL CHECK (length(Employee_Last_Name) <= 100 AND Employee_Last_Name GLOB '[A-Za-z]*'),
                    Employee_Phone TEXT CHAR(20) NOT NULL UNIQUE CHECK (Employee_Phone GLOB '04[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]') 
                    )''')
    
    cursor.execute('''CREATE TABLE IF NOT EXISTS Payment (
                    Payment_ID TEXT CHAR(20) PRIMARY KEY CHECK (Payment_ID GLOB 'P[0-9][0-9][0-9][0-9][0-9]'),
                    Payment_Method TEXT CHAR(50) NOT NULL CHECK (length(Payment_Method) <= 50 AND Payment_Method GLOB '[A-Za-z ]*'),
                    Payment_Status TEXT CHAR(50) NOT NULL CHECK (length(Payment_Status) <= 50 AND Payment_Status IN ('Paid', 'Unpaid', 'Partially Paid')),
                    Booking_ID TEXT CHAR(20) NOT NULL,
                    FOREIGN KEY (Booking_ID) REFERENCES Booking(Booking_ID),
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS Job (
                    Job_ID TEXT CHAR(20) PRIMARY KEY CHECK (Job_ID GLOB 'J[0-9][0-9][0-9][0-9][0-9]'),
                    Job_Date DATE NOT NULL CHECK (Job_Date LIKE '__/__/____'),
                    Job_Status TEXT CHAR(20) NOT NULL CHECK (Job_Status IN ('Finished', 'Not Started', 'In Progress')),                        
                    Service_ID TEXT CHAR(20) NOT NULL,
                    Employee_ID TEXT CHAR(20) NOT NULL,
    ...
    if choice == '1': 
        ...
        try:
            ...
            if "UNIQUE constraint failed" in str(e):
                ...
                print("\nOne of the following constraints has failed:")
                print("Customer ID must start with 'C' followed by 5 digits (e.g., C12345).")
                print("First Name must contain only letters and be below 101 characters.")
                print("Last Name must contain only letters and be below 101 characters.")
                ...
                print("Email must be in local-part@domain.tld format (e.g., john@gmail.com) and be below 101 characters.")
            else:
        ...
        try:
            ...
            if "UNIQUE constraint failed" in str(e):
                ...
                print("Service Type must contain only letters and spaces, and be below 51 characters.")
        ...
        try:
            ...
            if "UNIQUE constraint failed" in str(e):
                ...
                print("Payment Method must contain only letters and spaces, and be below 51 characters.")
...
def view_records():
    ...
    if choice == '1':  
        ...
        rows = cursor.fetchall()
        print("\nBooking Records:")
        print("Booking ID | Booking Date | Customer ID")
        for row in rows:
            print(row)
        ...
        rows = cursor.fetchall()
        print("\nService Records:")
        print("Service ID | Type | Cost ($) | Duration (mins)| Booking ID")
        for row in rows:
            print(row)
        ...
        rows = cursor.fetchall()
        print("\nPayment Records:")
        print("Payment ID | Method | Status | Booking ID")
        for row in rows:
            print(row)
        ...
        rows = cursor.fetchall()
        print("\nJob Records:")
        print("Job ID | Date | Status | Service ID | Employee ID")
        for row in rows:
            print(row)
...
def delete_record():
    ...
    if choice == '1':
        ...
        if cursor.fetchone() is None:
            print("Customer ID not found. No record deleted.")
            return
        try:
            ...
            print("Could not delete customer record.")
            if "FOREIGN KEY constraint failed" in str(e):
                print("This record is referenced in the Booking table.")
            else:
                print(f"Database error: {e}")
        ...
        if cursor.fetchone() is None:
            print("Booking ID not found. No record deleted.")
            return
        try:
            ...
            print("Could not delete booking record.")
            if "FOREIGN KEY constraint failed" in str(e):
                print("This record is referenced in the Service and/or Payment tables.")
            else:
                print(f"Database error: {e}")
        ...
        if cursor.fetchone() is None:
            print("Service ID not found. No record deleted.")
            return
        try:
            ...
            print("Could not delete service record.")
            if "FOREIGN KEY constraint failed" in str(e):
                print("This record is referenced in the Job table.")
            else:
                print(f"Database error: {e}")
        ...
        if cursor.fetchone() is None:
            print("Employee ID not found. No record deleted.")
            return
        try:
        ...
        if cursor.fetchone() is None:
            print("Payment ID not found. No record deleted.")
            return
        else:
        ...
        if cursor.fetchone() is None:
            print("Job ID not found. No record deleted.")
            return
        else:
...
def generate_report():
    print("\nSelect Table to Update Record:")
    print("1. Unpaid Jobs")
    print("2. Employee Schedules")
    print("3. Services In Bookings")
    print("4. Total Booking Cost & Duration")
    print("5. Bookings By Customers")
    print("6. Paid Bookings")
    print("7. Back to Main Menu")
    choice = input("Enter your choice (1-7): ")
    ...
    if choice == '1':  
        query = """
                ...
                GROUP BY
                    ...
                    Booking.Booking_Date,
                    Payment.Payment_Status
                HAVING Payment.Payment_Status = "Unpaid"
                ORDER BY
                    Booking.Customer_ID,
                    Booking.Booking_Date;
                """
        ...
        query = """
                ...
                INNER JOIN Booking ON Booking.Booking_ID = Service.Booking_ID
                ORDER BY
                    Employee.Employee_ID,
                    Booking.Booking_Date,
                    Booking.Booking_ID;
        ...
        print("Customer ID | Customer First Name | Customer Last Name | Customer Phone | Booking ID | Booking Date")
        for row in results:
            print(row)
